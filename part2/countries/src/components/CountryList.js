...
const CountryList = ({countries, setCountry, showCountry}) => {
       ...
       else if (countries.length > 1) { //to show a list of country names (country) + updated state (setCountry) + button to show more info about specific country (showCountry)
          return countries.map(country => <Country key={country.name} country={country} setCountry={setCountry} showCountry={showCountry}/>)
       } 
       else {
